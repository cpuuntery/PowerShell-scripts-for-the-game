...
# Define SendInput structures and function
$sendInputSignature = @'
using System;
...
function Send-MouseEvent {
    ...
    $input = New-Object SendInputWrapper+INPUT
    $input.type = [SendInputWrapper]::INPUT_MOUSE
    $mi = New-Object SendInputWrapper+MOUSEINPUT
    $mi.dx = $dx
    $mi.dy = $dy
    $mi.mouseData = 0
    $mi.dwFlags = $flags
    ...
    $input.mi = $mi
    [SendInputWrapper]::SendInput(1, @($input), [System.Runtime.InteropServices.Marshal]::SizeOf($input)) | Out-Null
}
...
function Click-AtPosition {
    ...
    # Perform a left-click (down then up) using SendInput
    Send-MouseEvent -dx 0 -dy 0 -flags ([SendInputWrapper]::MOUSEEVENTF_LEFTDOWN)
    Send-MouseEvent -dx 0 -dy 0 -flags ([SendInputWrapper]::MOUSEEVENTF_LEFTUP)
}
