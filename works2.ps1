...
# Define SendInput and supporting structures (namespace style, correct fields/constants)
$signature = @'
using System;
using System.Runtime.InteropServices;

namespace Win32Functions {
    public class Win32SendInputNew {
...
# Helper to send a mouse event
function Send-MouseEvent {
    param (
        [int]$flags
    )
    $input = New-Object Win32Functions.Win32SendInputNew+INPUT
    $input.type = [Win32Functions.Win32SendInputNew]::INPUT_MOUSE
    $mi = New-Object Win32Functions.Win32SendInputNew+MOUSEINPUT
    $mi.dx = 0
    $mi.dy = 0
    $mi.mouseData = 0
    $mi.dwFlags = $flags
    ...
    $cbSize = [System.Runtime.InteropServices.Marshal]::SizeOf($input)
    [Win32Functions.Win32SendInputNew]::SendInput(1, @($input), $cbSize) | Out-Null
}
...
function Click-AtPosition {
    ...
    # Send DOWN and UP individually for robustness
    Send-MouseEvent -flags ([Win32Functions.Win32SendInputNew]::MOUSEEVENTF_LEFTDOWN)
    Send-MouseEvent -flags ([Win32Functions.Win32SendInputNew]::MOUSEEVENTF_LEFTUP)
}
