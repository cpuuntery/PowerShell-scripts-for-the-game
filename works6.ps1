...
# Define SendInput and supporting structures with proper namespace
$signature = @'
using System;
...
Add-Type -MemberDefinition $signature -Name "Win32SendInput" -Namespace Win32Functions -PassThru | Out-Null

# Helper function to send individual mouse events
function Send-MouseEvent {
    ...
    # Get correct struct size from live instance
    $structSize = [System.Runtime.InteropServices.Marshal]::SizeOf($input)
    
    [Win32Functions.Win32SendInput]::SendInput(1, @($input), $structSize) | Out-Null
}
...
function Click-AtPosition {
    ...
    # Send left down and up events separately
    Send-MouseEvent -flags ([Win32Functions.Win32SendInput]::MOUSEEVENTF_LEFTDOWN)
    Send-MouseEvent -flags ([Win32Functions.Win32SendInput]::MOUSEEVENTF_LEFTUP)
}
